# ReviTchucky Development Guidelines
...
## Project Overview
ReviTchucky is a Revit add-in toolkit for family management. Phase 1 focuses on building a Family Library Indexer that scans a folder of .rfa files, extracts metadata (category, parameters, thumbnails), and stores it in a shared SQLite database.

## Architecture Principles

### Three-Project Structure
- **ReviTchucky.Core**: Plain .NET class library. Zero Revit API references. Contains data models, SQLite schema, OLE document operations, metadata extraction logic, and configuration management. This is the reusable engine.
- **ReviTchucky.UI**: WPF class library with MVVM pattern. Zero Revit API references. Contains Views (SettingsWindow, IndexProgressWindow) and ViewModels. Modeless windows that can be hosted either in a dockable pane or floated independently.
- **ReviTchucky.Revit**: The actual Revit add-in. Implements `IExternalApplication`. Owns the ribbon, external commands, and external event handlers. Acts as the bridge between UI and Revit API. Never calls Revit API from background threads.
...
- `Deploy.ps1` handles copying the right binary set to `C:\ProgramData\Autodesk\Revit\Addins\<version>\ReviTchucky\`
...
- Settings stored in JSON at `%AppData%\Autodesk\Revit\Addins\ReviTchucky\config.json`
...
- File location: `%AppData%\Autodesk\Revit\Addins\ReviTchucky\config.json`
...
### Revit Add-In (ReviTchucky.Revit)

#### External Application & Ribbon
- `IExternalApplication.OnStartup()`: creates ribbon tab "ReviTchucky" with two buttons
...
- Run `dotnet build ReviTchucky.sln -c Release2023|Release2024|Release2025` to build a given target
- Run `Deploy.ps1` (as Administrator) to copy binaries and manifests to `C:\ProgramData\Autodesk\Revit\Addins\`
- Restart Revit
- Check the ribbon for the "ReviTchucky" tab and "Settings" / "Index Library" buttons
